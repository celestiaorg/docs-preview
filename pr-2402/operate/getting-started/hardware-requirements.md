...
| Bridge node (`--archival`)    | 64 GB RAM  | 32 cores    | 637 TiB NVME\* | 1 Gbps    |
...
To run an archival bridge node, start with a fresh store and pass `--archival`
every time you run `celestia bridge start`. A store that has already pruned data
cannot switch back to archival mode.
