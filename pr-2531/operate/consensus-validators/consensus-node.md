...
### Recommended block retention for pruned nodes

The default `min-retain-blocks` value is `3000`. For non-archive nodes that
can prune historical block data but need a longer retention window, set
`min-retain-blocks = 600000` in your `app.toml`. This keeps a larger recent
block history without retaining all block data:
