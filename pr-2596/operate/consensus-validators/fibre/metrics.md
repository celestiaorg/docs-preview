...
such as Grafana Alloy or the OTel Collector. Configure the collector to receive
both signals and forward them to your tracing and metrics backends.
...
`/otel/v1/traces` and `/otel/v1/metrics`. Remove either signal suffix from an
existing endpoint when upgrading.
