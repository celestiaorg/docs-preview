...
Starting with celestia-app v10.2, use `config sync` to add missing fields and
...
### RPC and storage settings in v10
...
The RPC setting limits concurrent heavy requests. Blockstore compaction is
optional and does not enable pruning. Stop the node before running one-off
compaction of an existing pruned backlog.
