...
> **Note:** The instructions for this tutorial are for a **continuous** vesting account. To create a delayed vesting account, add the `--delayed` flag to your vesting transaction.

## Local devnet

[download and install celestia-app](/operate/consensus-validators/install-celestia-app),
selecting the
[network and corresponding version](/operate/networks/overview)
...
```bash
cd $HOME/celestia-app/build
./celestia-appd keys add vesting-key --home $CELESTIA_APP_HOME --keyring-backend test
```
...
```bash
./celestia-appd keys list --home $CELESTIA_APP_HOME --keyring-backend test
```
...
> **Note:** The remainder of the instructions are for a **continuous** vesting account. To create a delayed vesting account, use the `--delayed` flag. For example:  
> `./celestia-appd tx vesting create-vesting-account $TO_ADDRESS 100000utia 1686748051 --from $FROM_ADDRESS --gas auto --fees 100000utia --chain-id private --home $CELESTIA_APP_HOME --keyring-backend test --delayed`

```bash
./celestia-appd tx vesting create-vesting-account $TO_ADDRESS 100000utia 1686748051 --from $FROM_ADDRESS --gas auto --fees 100000utia --chain-id private --home $CELESTIA_APP_HOME --keyring-backend test
```

Select "Y" to choose "yes".

> **Optional:** Add `-y` to run the transaction without an interactive prompt:  
> `./celestia-appd tx vesting create-vesting-account $TO_ADDRESS 100000utia 1686748051 --from $FROM_ADDRESS --gas auto --fees 100000utia --chain-id private --home $CELESTIA_APP_HOME --keyring-backend test -y`
...
[installed celestia-app for the latest version for Mocha testnet](/operate/consensus-validators/install-celestia-app).

### Create a wallet

Use the `test` backend only for these Mocha examples. It stores keys unencrypted
on disk. Pass it explicitly rather than changing the default backend used by
other commands:
...
```bash
celestia-appd keys add origin --keyring-backend "$KEYRING_BACKEND"
celestia-appd keys add vesting --keyring-backend "$KEYRING_BACKEND"
```
...
Head to [the faucet](/operate/networks/mocha-testnet#mocha-testnet-faucet),
...
```bash
celestia-appd tx vesting create-vesting-account $TO_ADDRESS 100000utia 1686748051 --from $FROM_ADDRESS --gas 100000 --fees 100000utia --node $RPC_URL --chain-id mocha --keyring-backend "$KEYRING_BACKEND" --delayed
```
...
You can set up a [validator](/operate/consensus-validators/validator-node)
or [consensus node](/operate/consensus-validators/consensus-node)
...
to run each flag for every command line that you use. Keep your existing
`keyring-backend` setting; the Mocha commands above select their backend explicitly.
